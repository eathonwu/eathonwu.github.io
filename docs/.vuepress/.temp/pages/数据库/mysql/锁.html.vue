<template><div><h1 id="概览" tabindex="-1"><a class="header-anchor" href="#概览"><span>概览</span></a></h1>
<figure><img src="https://cdn.xiaolincoding.com//mysql/other/1e37f6994ef44714aba03b8046b1ace2.png" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure>
<h1 id="全局锁" tabindex="-1"><a class="header-anchor" href="#全局锁"><span>全局锁</span></a></h1>
<p>示例:</p>
...
<span class="line"><span style="--shiki-light:#A626A4;--shiki-dark:#C678DD"> unlock</span><span style="--shiki-light:#383A42;--shiki-dark:#ABB2BF"> tables </span><span style="--shiki-light:#A0A1A7;--shiki-light-font-style:italic;--shiki-dark:#7F848E;--shiki-dark-font-style:italic">--解锁</span></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>用途: 全库备份,期间不允许数据修改</p>
<h1 id="表级别锁" tabindex="-1"><a class="header-anchor" href="#表级别锁"><span>表级别锁</span></a></h1>
<h2 id="_1-表锁" tabindex="-1"><a class="header-anchor" href="#_1-表锁"><span>1.表锁</span></a></h2>
<div class="language-sql line-numbers-mode" data-highlighter="shiki" data-ext="sql" data-title="sql" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34"><pre v-pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"></span>
...
<h2 id="_3-意向锁" tabindex="-1"><a class="header-anchor" href="#_3-意向锁"><span>3.意向锁</span></a></h2>
...
<font color=red>**意向锁的目的是为了快速判断表里是否有记录被加锁**。</font><h2 id="_4-auto-inc-锁" tabindex="-1"><a class="header-anchor" href="#_4-auto-inc-锁"><span>4.AUTO-INC 锁</span></a></h2>
<p>里的主键通常都会设置成自增的，这是通过对主键字段声明 <code v-pre>AUTO_INCREMENT</code> 属性实现的。</p>
...
<p><strong>在插入数据时，会加一个表级别的 AUTO-INC 锁</strong>，然后为被 <code v-pre>AUTO_INCREMENT</code> 修饰的字段赋值递增的值，等插入语句执行完成后，才会把 AUTO-INC 锁释放掉。</p>
<p>那么，一个事务在持有 AUTO-INC 锁的过程中，其他事务的如果要向该表插入语句都会被阻塞，从而保证插入数据时，被 <code v-pre>AUTO_INCREMENT</code> 修饰的字段的值是连续递增的。</p>
<p>但是， AUTO-INC 锁再对大量数据进行插入的时候，会影响插入性能，因为另一个事务中的插入会被阻塞。</p>
<h1 id="行级锁" tabindex="-1"><a class="header-anchor" href="#行级锁"><span>行级锁</span></a></h1>
<h2 id="记录锁record-lock" tabindex="-1"><a class="header-anchor" href="#记录锁record-lock"><span>记录锁record lock</span></a></h2>
<h2 id="间隙锁gap-lock" tabindex="-1"><a class="header-anchor" href="#间隙锁gap-lock"><span>间隙锁gap lock</span></a></h2>
<h2 id="next-key-lock" tabindex="-1"><a class="header-anchor" href="#next-key-lock"><span>next-key lock</span></a></h2>
<h2 id="插入意向锁" tabindex="-1"><a class="header-anchor" href="#插入意向锁"><span>插入意向锁</span></a></h2>
